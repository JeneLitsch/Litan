...
namespace ltn::vm::inst {
	namespace {
		template<typename T>
		void * concat(const Value & l, const Value & r, Heap & heap) {
			const auto obj_l = heap.read<T>(l).get();
			const auto obj_r = heap.read<T>(r).get();
		...
		void * repeat(
			...
			const auto & str = heap.read<Container>(ref).get();
