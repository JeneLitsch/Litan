...
namespace ltn::c {
	...
	template<typename NodeT, auto body_fx>
	...
	ast::stmt_ptr parse_stmt_switch(Tokens & tokens) {
		if(auto begin = match(TT::SWITCH, tokens)) {
			auto sw1tch = parse_any_switch<ast::Switch, parse_statement>(tokens);
	...
	ast::expr_ptr parse_expr_switch(const Token & begin, Tokens & tokens) {
		auto sw1tch = parse_any_switch<ast::Choose, parse_expression>(tokens);
		if(sw1tch && !sw1tch->d3fault) {
